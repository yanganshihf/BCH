...
import com.integration.dao.SH_FH_HZDAO;
 ...
 */
@Controller
@RequestMapping("shfhhz/")
public class SH_FH_HZAction {
	@Autowired
	SH_FH_HZDAO pd;
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdShhzFh> resp = new JqgridPageResp<YhswxcBzdShhzFh>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows,String shi,String xian,String xiang,String yangdi_h,String shouhai_zl) {
		...
		try {
			...
			List<YhswxcBzdShhzFh> list = pd.queryList(shi,xian,xiang,yangdi_h,shouhai_zl);
			 // 记录数
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdShhzFh> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
