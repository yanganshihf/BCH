...
import com.integration.dao.CH_FH_HZDAO;
 ...
 */
@Controller
@RequestMapping("chfhhz/")
public class CH_FH_HZAction {
	@Autowired
	CH_FH_HZDAO pd;
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdChhzFh> resp = new JqgridPageResp<YhswxcBzdChhzFh>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows,String shi,String xian,String xiang,String yangdi_h,String shouhai_zl) {
		...
		try {
			...
			List<YhswxcBzdChhzFh> list = pd.queryList(shi,xian,xiang,yangdi_h,shouhai_zl);
			 // 记录数
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdChhzFh> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
