...
import com.integration.dao.BH_FH_HZDAO;
 ...
 */
@Controller
@RequestMapping("bhfhhz/")
public class BH_FH_HZAction {
	@Autowired
	BH_FH_HZDAO pd;
	@Autowired
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdBhhzFh> resp = new JqgridPageResp<YhswxcBzdBhhzFh>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows,String shi,String xian,String xiang,String yangdi_h,String shouhai_zl,String ganbing_zs) {
		...
		try {
			...
			List<YhswxcBzdBhhzFh> list = pd.queryList(shi,xian,xiang,yangdi_h,shouhai_zl,ganbing_zs);
			// 翻译字段
			for (YhswxcBzdBhhzFh yhswxcBzdBhhzFh : list) {
				yhswxcBzdBhhzFh.setWeihai_Cd(weihaiCdMap.get(yhswxcBzdBhhzFh.getWeihai_Cd()));
			} 
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdBhhzFh> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
