...
import com.integration.dao.SH_FQ_DCDAO;
...
@Controller
@RequestMapping("shfqdc/")
public class SH_FQ_DCAction {
	@Autowired
	SH_FQ_DCDAO pd;
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdShdcFq> resp = new JqgridPageResp<YhswxcBzdShdcFq>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows) {
		...
		try {
			...
			List<YhswxcBzdShdcFq> list = pd.queryList();
			 // 记录数
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdShdcFq> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
