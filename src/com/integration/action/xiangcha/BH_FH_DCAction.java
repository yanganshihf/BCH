...
import com.integration.dao.BH_FH_DCDAO;
...
@Controller
@RequestMapping("bhfhdc/")
public class BH_FH_DCAction {
	@Autowired
	BH_FH_DCDAO pd;
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdBhdcFh> resp = new JqgridPageResp<YhswxcBzdBhdcFh>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows) {
		...
		try {
			...
			List<YhswxcBzdBhdcFh> list = pd.queryList();
			 // 记录数
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdBhdcFh> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
