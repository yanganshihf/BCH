...
import com.integration.dao.CH_FH_DCDAO;
...
@Controller
@RequestMapping("chfhdc/")
public class CH_FH_DCAction {
	@Autowired
	CH_FH_DCDAO pd;
	...
	public Object delete(String uuid) {
		JqgridPageResp<YhswxcBzdChdcFh> resp = new JqgridPageResp<YhswxcBzdChdcFh>();
		// 验证参数
		try {
			pd.delete(uuid);
			resp.setMsg("删除成功");
	...
	public Object queryList(Integer page, Integer rows) {
		...
		try {
			...
			List<YhswxcBzdChdcFh> list = pd.queryList();
			 // 记录数
	...
	public Object findById(String uuid_bzd) {
		...
		// 验证参数
		try {
			List<YhswxcBzdChdcFh> list = pd.findById(uuid_bzd);
			resp.setRows(list);
			resp.setMsg("查询成功");
		} catch (Exception e) {
			// TODO: handle exception
