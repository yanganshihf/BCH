...
public interface SH_FH_DCDAO {
	@Delete("delete from YHSWXC_BZD_SHDC_FH where UUID = #{0}")
	void delete(String uuid);
	
	@Select("select * from  YHSWXC_BZD_SHDC_FH")
	List<YhswxcBzdShdcFh> queryList();
	
	@Select("select * from YHSWXC_BZD_SHDC_FH where  "
			+ "UUID = #{0}")
	List<YhswxcBzdShdcFh> findById(String uuid_bzd);

	@Update("update YHSWXC_BZD_SHDC_FH set uuid_bzd =#{1},yangf_h=#{2},tuqiu_gs =#{3},"
