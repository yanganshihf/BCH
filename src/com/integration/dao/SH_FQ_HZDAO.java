 ...
 *
 */
public interface SH_FQ_HZDAO {
	@Delete("delete from YHSWXC_BZD_SHHZ_FQ where UUID = #{0}")
	void delete(String uuid);
	
	@Select("select t0.SHI,t0.XIAN,t0.YANGDI_H,t1.* from YHSWXC_BZD t0,YHSWXC_BZD_SHHZ_FQ t1 \r\n" + 
	...
	List<YhswxcBzdShhzFq> queryList(String shi, String xian, String xiang, String yangdi_h, String shouhai_zlStr);
	
	@Select("select t0.SHI,t0.XIAN,t0.YANGDI_H,t1.* from YHSWXC_BZD t0,YHSWXC_BZD_SHHZ_FQ t1 \r\n" + 
			"WHERE t0.UUID_BZD = t1.UUID_BZD  "
			+ "and t1.uuid = #{0}")
	List<YhswxcBzdShhzFq> findById(String uuid);
	
	@Update("update YHSWXC_BZD_SHHZ_FQ set uuid_bzd =#{1},yhsw_lb=#{2},yhsw_mc =#{3},"
			...
			+ "tuqiu_xs =#{18},weihai_cd=#{19},daibiao_mj=#{20},chengzai=#{21},diaocha_ry=#{22}"
			+ ",last_changetime=#{23}"
			+ " where uuid = #{0}")
