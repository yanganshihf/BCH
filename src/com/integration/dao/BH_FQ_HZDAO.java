...
public interface BH_FQ_HZDAO {
	@Delete("delete from YHSWXC_BZD_BHHZ_FQ where UUID = #{0}")
	void delete(String uuid);
	
	@Select("select t0.SHI,t0.XIAN,t0.YANGDI_H,t1.* from YHSWXC_BZD t0,"
			+ "YHSWXC_BZD_BHHZ_FQ t1 \r\n" + 
			" WHERE t0.UUID_BZD = t1.UUID_BZD  "
			+ "and t0.SHI like #{0} and t0.XIAN like #{1}  "
			+ "and t1.XIANG like #{2} and t0.YANGDI_H like #{3} "
			+ "and t1.SHOUHAI_ZL like #{4} and t1.GANBING_ZS like #{5}")
	List<YhswxcBzdBhhzFq> queryList(String shi, String xian, String xiang, String yangdi_h, String shouhai_zlStr, String ganbing_zsStr);
	
	@Select("select t0.SHI,t0.XIAN,t0.YANGDI_H,t1.* from YHSWXC_BZD t0,YHSWXC_BZD_BHHZ_FQ t1 \r\n" + 
			"WHERE t0.UUID_BZD = t1.UUID_BZD  "
			+ "and t1.UUID_BZD = #{0}")
	List<YhswxcBzdBhhzFq> findById(String uuid_bzd);

	@Update("update YHSWXC_BZD_BHHZ_FQ set uuid_bzd =#{1},yhsw_lb=#{2},yhsw_mc =#{3},"
