...
layui.use('table', function(){
	  var closData =  [[
	      ...
	      ,{field:'code',title: '虫害代码',align:'center'}
	      ,{field:'name',title: '虫害名称',align:'center'}
	      ,{field:'ladingM',title: '拉丁名称',align:'center'}
	      ,{field:'jiBie',title: '虫害级别',align:'center'}
	  ...
	  table.render({
	    elem: '#table'
	    ,url:'/ResourceMonitor/codemanage/queryChonghaiCode'
	    ,cellMinWidth: 60 
	    ,cols:closData
	  	,id: 'tableId'
	  	,title:'虫害编码'
 			...
 			layer.open({
 			    ...
 			    yes: function(index, layero){
 					...
 					if(code==''){
		 	 		   ...
		 	 		   $.post("/ResourceMonitor/codemanage/addChonghaiCode",
 	 	 			   data,
	  ...
	  //删除
 	  $("#deleteButton").off().on("click",function(){
 		 ...
 		 if(data.length>0){
 	 		 ...
 	 		 for(var i = 0;i<data.length;i++){
 	 		  ...
 	 		  $.post("/ResourceMonitor/codemanage/deleteChonghaiCode",
 			  {
 			    ids:ids.join(','),
 			  },
 			  function(data,status){
 	  ...
 	  $("#exportButton").off().on("click",function(){
		    $.post("/ResourceMonitor/codemanage/exportChonghaiCode",
    	    {
	            name:$("#nameField").val(),
	            code:$("#codeField").val(),
    	    },
