...
layui.use('table', function(){
	  var closData =  [[
	      ...
	      ,{field:'code',title: '鼠害代码',align:'center'}
	      ,{field:'name',title: '鼠害名称',align:'center'}
	      ,{field:'ladingM',title: '拉丁名称',align:'center'}
	      ,{field:'jiBie',title: '鼠害级别',align:'center'}
	  ...
	  table.render({
	    elem: '#table'
	    ,url:'/ResourceMonitor/codemanage/queryShuhaiCode'
	    ,cellMinWidth: 60 
	    ,cols:closData
	  	,id: 'tableId'
	  	,title:'鼠害编码'
 			...
 			layer.open({
 			    ...
 			    yes: function(index, layero){
 					...
 					if(code==''){
		 	 		   ...
		 	 		   $.post("/ResourceMonitor/codemanage/addShuhaiCode",
 	 	 			   data,
	  ...
	  //删除
 	  $("#deleteButton").off().on("click",function(){
 		 ...
 		 if(data.length>0){
 	 		 ...
 	 		 for(var i = 0;i<data.length;i++){
 	 		  ...
 	 		  $.post("/ResourceMonitor/codemanage/deleteShuhaiCode",
 			  {
 			    ids:ids.join(','),
 			  },
 			  function(data,status){
 	  ...
 	  $("#exportButton").off().on("click",function(){
		    $.post("/ResourceMonitor/codemanage/exportShuhaiCode",
    	    {
	            name:$("#nameField").val(),
	            code:$("#codeField").val(),
    	    },
