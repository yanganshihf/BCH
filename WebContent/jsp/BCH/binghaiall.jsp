...
<body >
	<!-- <div style="width: 100%;margin-top: 2%;margin-left: 3%"> 
		<span>:<span><input	style=" width: 10%"/></span></span> 
		<span style="margin-left: 2%"><span><input style=" width: 10%"/></span></span>
		
		<div style="margin-left: -2%;margin-top: 2%">
			<table id="binghaiall"></table>
			<div id="page" style="position: fixed; bottom: 0px;"></div>
		</div>
	</div> -->
		<div class="layui-form-item" style="margin-top: 10px">
		<div class="layui-inline">
			<label class="layui-form-label">感染指数</label>
			<div class="layui-input-inline">
				<input type="text" name="phone" id="" autocomplete="off" class="layui-input">
			</div>
		</div>
		<div class="layui-inline">
	...
	$(function() {
		...
		$(window).resize(function() {
			$("#binghaiall").setGridWidth($(window).width() - 35);
			$("#binghaiall").setGridHeight($(window).height()-180);
		});
	});
	function diaocha(tit){
		t = tit;
		if(t=="病害防前调查记录表"){
			url = localhostPath + projectName + '/shfqhdc/queryList';
			jQuery("#binghaiall").jqGrid(
				      {
				        url : url,
				    	rowNum : 10,
					    rowList : [ 10, 20, 30 ],
				        ...
				        datatype : "json",
				        loadonce: true,
				        colNames : [ 'uuid','样方', '样株数', '病级', '样地号取值', '病情','备注', '编辑'],
				        colModel : [ 
				                     {name : 'uuid',index : 'uuid',width : 63,sortable : false,hidden : true}, 
				                     {name : 'yangf_H',index : 'yangf_H',width : 55}, 
				                     {name : 'yangz_H',index : 'yangz_H, invdate',width : 55}, 
				                     {name : 'bing_Ji',index : 'bing_Ji',width : 70,align : "right"}, 
				                     {name : 'daibiaozhi',index : 'daibiaozhi',width : 70,align : "right"}, 
				                     {name : 'bing_Qing',index : 'bing_Qing',width : 100,align : "right"}, 
				                     {name : 'bei_Zhu',index : 'bei_Zhu',width : 100,align : "right"}, 
				                     {name : 'bianji',index : 'bianji',width : 100,sortable : false},
				                   ],
						jsonReader : {
								root : "rows",
								records : "records",
							...
							gridComplete: function () {
								...
								var ids = $("#binghaiall").getDataIDs();
								// 遍历获取每行数据
								for (var i = 0; i < ids.length; i++) {
								// 点击根据id
									$("#binghaiall").setCell(ids[i],"bianji","<button class='btn btn-danger' onclick ='edit("+ ids[i] +")'>编辑</ button>");
								}
							}
				      });
		}else{
			...
			jQuery("#binghaiall").jqGrid(
				      {
						...
						jsonReader : {
							...
							gridComplete: function () {
								$("#binghaiall").setGridWidth($(window).width() - 25);
								$("#binghaiall").setGridHeight($(window).height()-200);
								var ids = $("#binghaiall").getDataIDs();
								// 遍历获取每行数据
								for (var i = 0; i < ids.length; i++) {
								// 点击根据id
	...
	function edit(id){
		var rowData = $("#binghaiall").jqGrid("getRowData", id);// 根据上面的id获得本行的所有数据
		var msg = t;
		if(msg=="病害防前调查记录表"){
