...
window.setTimeout(function() {
	...
	if(value == 3){
		  $("#unEmployDate").prop("disabled",false);
		  $("#unemployReason").prop("disabled",false);
		  $("#hideThis").css("background-color","transparent");
		  $("#unemployReason").css("color","#000");
		  $("#unEmployDate").css("color","#000");
		  laydate.render({
				elem : '#unEmployDate' // 指定元素
				,
				showBottom : false
			});
	  }else{
		  $("#unEmployDate").prop("disabled",true);
		  $("#unemployReason").prop("disabled",true);
...
$("#employState").on("change",function(){
	var selectValue = $("#employState").val();
	changeEmploy(selectValue);
});

function changeEmploy(value){
	  if(value == 3){
		  var now = new Date();
