...
WKTUtil.prototype = {
    ...
    parse: {
        ...
        'point': function(str) {
        	...
        	var Ncoords=coords.map(function(data){
        		return Number(data);
        	}); 
            return Ncoords;//new esri.geometry.Point(coords[0], coords[1]);
        },
        ...
        'multipolygon': function(str) {
            var ring,ring0, linestring, linearring;
            var rings0 = this.trim(str).replace(this.regExes.trimParens, '$1');  
            var rings=this.trim(rings0).split('), (');
            var components = []; 
            for(var i=0, len=rings.length; i<len; ++i) {
                ...
                ring = ring0.replace(this.regExes.trimParens, '$1');
                linestring = this.parse.linestring.apply(this, [ring]); 
                components.push(linestring);
            }
            return components;
